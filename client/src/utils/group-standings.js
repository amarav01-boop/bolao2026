...
export function calculateGroupStandings(matches = [], scoreResolver) {
  const table = new Map();
  ...
  matches.forEach((match) => {
    ...
    const homeScore = normalizeScore(scores?.homeScore);
    const awayScore = normalizeScore(scores?.awayScore);
...
export function hasCompleteScores(matches = [], scoreResolver) {
  return matches.length > 0 && matches.every((match) => {
    const scores = scoreResolver(match);
    return normalizeScore(scores?.homeScore) !== null && normalizeScore(scores?.awayScore) !== null;
  });
}

export function calculateGroupClassificationPoints(predictedStandings = [], realStandings = []) {

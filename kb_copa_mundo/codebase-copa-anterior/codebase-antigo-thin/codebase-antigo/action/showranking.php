...
/*$sql = "SELECT USERS.USERID, USERS.USERNAME, USERS.USERLOCATION, USERS.USERNICKNAME, max(RANKING.RANKING) as 
CURRENT_POS, max(RANKING.LAST_RANKING) AS PREVIOUS_POS, max(RANKING.RANKING) - max(RANKING.LAST_RANKING) as POS_DIFF, SUM(RANKING.ROUND_PTS) AS TOTAL_PONTOS 
		...
		GROUP BY USERS.USERID, USERS.USERNAME, USERS.USERLOCATION, USERS.USERNICKNAME
		ORDER BY TOTAL_PONTOS DESC, 4, 2, 1";*/

/* query com pontos extras */
$sql= "SELECT USERS.USERID, USERS.USERNAME, USERS.USERLOCATION, USERS.USERNICKNAME, null as 
CURRENT_POS, null AS PREVIOUS_POS, null as POS_DIFF, SUM(vw_users_points.PONTOS) AS TOTAL_PONTOS 
		FROM vw_users_points, USERS
		WHERE USERS.USERID = vw_users_points.USERID
	 ...
	 while($row = $result->fetch_array(MYSQLI_ASSOC)){
			...
			echo "<tr class='special-tr'>";
				...
				if ($pontosposicao == $row["TOTAL_PONTOS"]){
					...
					echo "<td class='o-col-44px'>".$posicao.  "</td>";
				/*	echo ">>>  ";
					echo $difposicao;
					echo " | ";
					echo $posicao;
				...
				echo "<td class='o-col-352px'><strong>". $row["USERNICKNAME"]. "</strong><br>" . $row["USERNAME"]. "<br><i>".$row["USERLOCATION"]."</i></td>";
				//echo "<td class='o-col-352px'><strong>". $row["USERNICKNAME"]. "</strong><br><i>" .$row["USERLOCATION"]."</i></td>";

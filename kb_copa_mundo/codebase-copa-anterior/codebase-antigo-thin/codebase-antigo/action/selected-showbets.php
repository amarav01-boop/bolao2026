...
MASTER.GAME_DATE, MASTER.GAME_STATUS FROM USERS, BETS, MASTER WHERE USERS.USERID = BETS.USERID AND USERS.USERID=". $selecteduser ." AND BETS.GAME_ID = MASTER.GAME_ID AND MASTER.GAME_ID <= 64 ORDER BY MASTER.PHASE_ID, MASTER.GAME_ID";
...
if ($result->num_rows > 0) {
	 while($row = $result->fetch_array(MYSQLI_ASSOC)){
			...
			if ($row["PHASE_ID"] != $grupo){
				...
				echo "<div id='layout-home-content-70'>";
					if ($selectedgroup == "I") {
						echo "<h5> OITAVAS DE FINAL</h5>";
					} elseif ($selectedgroup == "J") {
						echo "<h5> QUARTAS DE FINAL</h5>";
					} elseif ($selectedgroup == "K") {
						echo "<h5> SEMIFINAIS</h5>";
					} elseif ($selectedgroup == "L") {
						echo "<h5> FINAIS</h5>";
					}else {
						echo "<h5> GRUPO ". $row["PHASE_ID"] . "</h5>";
						include "./action/showgrouporder.php";
						include "./action/showgrouppoints.php";
					}
				echo "</div>";
			}	
				...
				if ($selectedgroup == "M") {
					...
					if ($selectedgroup == "XX") { echo " X "; } else {
				...
				if ($row["GAME_STATUS"] == 0){
					...
					echo utf8_encode($row["MASTER_SCORE_A"]) . " X ". utf8_encode($row["MASTER_SCORE_B"]);
				}
				echo "&nbsp";
				echo "|";
				echo "&nbsp";
	...
	echo "<h5> PALPITES EXTRA</h5>";
		include "./action/showsemiteampoints.php";
		include "./action/showstrikerpoints.php";
		include "./action/showstrikergoalspoints.php";
		include "./action/showchampionpoints.php";
		echo "<table>";
		echo "<tr>";
			echo "<th>";
			echo "CAMPEÃO";

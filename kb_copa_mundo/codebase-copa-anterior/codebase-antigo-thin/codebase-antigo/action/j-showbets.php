...
$sql = "SELECT MASTER.GAME_ID, MASTER.PHASE_ID, MASTER.GAME_DATE, MASTER.TEAM_A, MASTER.TEAM_B, 
			...
			BETS.BET_POINTS FROM MASTER, BETS  WHERE MASTER.GAME_ID = BETS.GAME_ID AND BETS.USERID = " . $_SESSION["userid"] . " AND MASTER.PHASE_ID = 'J' ORDER BY MASTER.PHASE_ID, MASTER.GAME_ID";
...
if ($result->num_rows > 0) {
	 while($row = $result->fetch_array(MYSQLI_ASSOC)){
			...
			echo "<tr>";
				...
				//echo "<td class='o-col-44px'>". "<input type='text' maxlength=2 class= 'o-inputscore'  size=1 name='". $fieldnameA . "' value='".utf8_encode($row["SCORE_A"]) ."' />" .  "</td>";
				if ($row["SCORE_A"] == null){
					echo "<td class='o-col-44px'>". "<input type='text' maxlength=2 class= 'o-inputscore'  size=1 name='". $fieldnameA . "' value='0' />" .  "</td>";
				} else {
					echo "<td class='o-col-44px'>". "<input type='text' maxlength=2 class= 'o-inputscore'  size=1 name='". $fieldnameA . "' value='".utf8_encode($row["SCORE_A"]) ."' />" .  "</td>";
				}
				...
				if ($row["SCORE_B"] == null){
					echo "<td class='o-col-44px'>". "<input type='text' maxlength=2 class= 'o-inputscore'  size=1 name='". $fieldnameB . "' value='0' />" .  "</td>";
				} else {
